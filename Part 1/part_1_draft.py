    ...
    for blob in blobs:
        if blob.name.endswith('.csv'):
            ...
            if 'SCRAPED_DATE' in df.columns:
                df['SCRAPED_DATE'] = df['SCRAPED_DATE'].apply(transform_date)

            if 'HOST_SINCE' in df.columns:
                df['HOST_SINCE'] = df['HOST_SINCE'].apply(transform_date)
...
[census_data_01_task, census_data_02_task, listings_data_files, nsw_l_code_task]
